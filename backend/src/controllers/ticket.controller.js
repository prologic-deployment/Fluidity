...
function notifier(sujet, heading, bodyHtml) {
  ...
  sendSupportEmail(sujet, html).catch(console.error);
}
...
const getAllTickets = async (req, res) => {
  try {
    ...
    const filtre = { ...filtreProprietaire(req) };
...
const getAssignees = async (req, res) => {
  try {
    ...
    const users = await Utilisateur.find({ role: { $in: ROLES_SUPPORT } })
...
const getTicketById = async (req, res) => {
  try {
    const ticket = await populateTicket(
      Ticket.findOne({ _id: req.params.id, ...filtreProprietaire(req) })
    );
    if (!ticket) {
...
const updateTicket = async (req, res) => {
  try {
    const ticket = await Ticket.findOne({ _id: req.params.id, ...filtreProprietaire(req) });
    if (!ticket) {
      res.status(404).json({ message: 'Ticket introuvable' });
      return;
    }
...
const assignerTicket = async (req, res) => {
  try {
    ...
    if (req.body.assignedTo !== undefined) {
      ...
      if (id) {
        const tech = await Utilisateur.findOne({ _id: id, role: { $in: ROLES_SUPPORT } });
        if (!tech) {
          res.status(400).json({ message: 'Technicien introuvable.' });
          return;
        }
...
const changerStatutTicket = async (req, res) => {
  try {
    ...
    await enregistrerActivite(
      ticket,
      nouveauStatut === 'Résolu' ? 'resolution' : nouveauStatut === 'Réouvert' ? 'reouverture' : nouveauStatut === 'Clôturé' ? 'cloture' : 'statut',
      req,
      { de: statutActuel, vers: nouveauStatut, motif: motif || undefined }
    );
