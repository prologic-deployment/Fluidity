 ...
 *   - à défaut, dérivation scrypt de JWT_SECRET (développement).
 *
...
function getKey() {
  ...
  if (!warnedFallback) {
    console.warn(
      '[Fluidity] TWO_FACTOR_ENCRYPTION_KEY non définie (≥ 32 caractères) — ' +
        'les secrets 2FA sont chiffrés avec une clé dérivée de JWT_SECRET. ' +
        'Définissez une clé dédiée en production (.env).'
    );
    warnedFallback = true;
  }
  const base = process.env.JWT_SECRET || 'fluidity_dev_jwt_secret';
  cachedKey = crypto.scryptSync(base, 'fluidity-2fa-key-derivation', 32);
  return cachedKey;
}
