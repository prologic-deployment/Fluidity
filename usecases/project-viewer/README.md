...
Scope: product role of **project_management** (project rank 0).
...
- **Tasks**: List / board / get / backlog tasks; Watch / unwatch task
- **Milestones**: List milestones
- **Sprints**: List sprints
- **Comments**: Read comments; Edit / delete comment (own comments only (rank < 5))
- **Files**: List files
...
- Full task update (PUT) — lacks project.task.update (API 403; UI tab still visible, guard is product-level)
- Change task status (transition/move) — lacks project.task.update (API 403; UI tab still visible, guard is product-level)
- Edit task checklist — lacks project.task.update (API 403; UI tab still visible, guard is product-level)
...
- Post comment (+ @mentions) — rank 0 < 1 (API 403; UI tab still visible, guard is product-level)
- Upload file — rank 0 < 1 (API 403; UI tab still visible, guard is product-level)
- Delete file — rank 0 < 3 (API 403; UI tab still visible, guard is product-level)
...
- Submit deliverable — lacks project.task.update (API 403; UI tab still visible, guard is product-level)
- Approve / reject deliverable — lacks project.task.update (API 403; UI tab still visible, guard is product-level)
...
- IMPLEMENTED (allowed + UI): 20
- BACKEND_ONLY (allowed, no UI): 1
- Restrictions verified (denied by design): 26
