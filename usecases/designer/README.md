...
Scope: product role of **project_management** (project rank 2).
...
- **Time**: List time entries; Log time; Edit / delete time entry (own entries only (rank < 3))
- **Deliverables**: List deliverables; Submit deliverable; Approve / reject deliverable (submit-only path (rank < 4 cannot approve/reject))
...
- Delete file — rank 2 < 3 (API 403; UI tab still visible, guard is product-level)
...
- IMPLEMENTED (allowed + UI): 28
- BACKEND_ONLY (allowed, no UI): 1
- Restrictions verified (denied by design): 18
