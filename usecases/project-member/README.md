...
Scope: product role of **project_management** (project rank 1).
...
- **Projects**: List projects; View global dashboard; View personal dashboard (my tasks); Cross-project search; View project detail; View project dashboard tab; View project calendar; Read workflow config; Read activity journal
- **Members**: List project members (+license flags)
- **Tasks**: List / board / get / backlog tasks; Full task update (PUT) (assignee-only (rank < 2)); Change task status (transition/move); Edit task checklist; Watch / unwatch task
- **Milestones**: List milestones
- **Sprints**: List sprints
- **Comments**: Read comments; Post comment (+ @mentions); Edit / delete comment (own comments only (rank < 5))
- **Files**: List files; Upload file
- **Time**: List time entries; Edit / delete time entry (own entries only (rank < 3))
- **Deliverables**: List deliverables; Approve / reject deliverable (submit-only path (rank < 4 cannot approve/reject))
...
- View project reports — lacks project.report.read (API 403; UI tab still visible, guard is product-level)
...
- Create / start / complete / plan / delete sprint — lacks project.sprint.manage (API 403; UI tab still visible, guard is product-level)
...
- Delete file — rank 1 < 3 (API 403; UI tab still visible, guard is product-level)
- Log time — rank 1 < 2 (API 403; UI tab still visible, guard is product-level)
- Submit deliverable — rank 1 < 2 (API 403; UI tab still visible, guard is product-level)
...
- IMPLEMENTED (allowed + UI): 26
- BACKEND_ONLY (allowed, no UI): 1
- Restrictions verified (denied by design): 20
