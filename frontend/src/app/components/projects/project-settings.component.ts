...
import { Project, WorkflowState } from '../../models/project.model';
import { I18N_IMPORTS } from '../../i18n/i18n.pipe';
import { METHODOLOGIES, PRIORITIES, PROJECT_STATUSES } from './project.constants';
...
export class ProjectSettingsComponent implements OnInit, OnDestroy {
  ...
  // Champs généraux
  form = { name: '', description: '', stakeholder: '', status: 'planning', priority: 'medium', visibility: 'team', startDate: '', endDate: '' };
  // Workflow personnalisé
  wfStates: { key: string; label: string; color: string; terminal: boolean }[] = [];
  wfDirty = false;
  // Règles de santé
    ...
    this.route.parent?.params
      ...
      .subscribe({
        next: (r) => {
          ...
          this.wfStates = (r.workflow.states || []).map((s) => ({ key: s.key, label: s.label || '', color: s.color || '', terminal: !!s.terminal }));
  ...
  addWfState(): void {
    this.wfStates.push({ key: `state_${Date.now().toString(36)}`, label: '', color: '', terminal: false });
    this.wfDirty = true;
  }
  ...
  resetWf(): void {
    this.wfStates = [
      { key: 'backlog', label: '', color: '', terminal: false },
      { key: 'todo', label: '', color: '', terminal: false },
      { key: 'in_progress', label: '', color: '', terminal: false },
      { key: 'blocked', label: '', color: '', terminal: false },
      { key: 'review', label: '', color: '', terminal: false },
      { key: 'completed', label: '', color: '', terminal: false },
      { key: 'cancelled', label: '', color: '', terminal: true },
    ];
    this.wfDirty = true;
  }
